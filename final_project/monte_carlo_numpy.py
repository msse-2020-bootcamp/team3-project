"""
Functions for running Monte Carlo Simulation by both Python Standard Library and NumPy.
"""
import math
import os
import random
...
def calculate_total_energy_np(coordinates, cutoff=3, box_length=None):
    """
    Calculate the total Lennard Jones energy of a system of particles by NumPy.
    ...
    for coordinate in coordinates:
        ...
        dist1 = calculate_distance_np(coordinate, coordinates, box_length)
        dist2 = dist1[dist1 != 0]
        dist3 = dist2[dist2 < cutoff]
        
        energy_i = calculate_LJ_np(dist3)
        energy_i_total = energy_i.sum()
        
        total_energy += energy_i_total
        
        coordinates = coordinates[1:num_atoms] 
...
def calculate_pair_energy_np(coordinates, i_particle, box_length, cutoff):
    ...
    r_ij = calculate_distance_np(coordinates, coordinates[i_particle], box_length)
    r_ij_1 = r_ij[r_ij > 0]
    r_ij_2 = r_ij_1[r_ij_1 < cutoff]
    e_pair = calculate_LJ_np(r_ij_2)
    e_total = e_pair.sum()
...
def run_simulation_np(coordinates, box_length, cutoff, reduced_temperature, num_steps, max_displacement = 0.1, freq = 1000):
    ...
    try:
        total_energy = calculate_total_energy_np(coordinates, cutoff, box_length)
        print(total_energy)
    ...
    #print(calculate_tail_correction(cutoff, box_length, num_particles))
...
new_coords = run_simulation_np(coordinates_np, box_length, cutoff, reduced_temperature, num_steps, max_displacement, freq)
time_end = time.time()
time_pass = time_end - time_start


print('time cost of ' + str(num_steps)+' steps is ' +  str(time_pass) + 's.')